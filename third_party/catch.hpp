...
namespace Catch {
    ...
    void AssertionHandler::complete() {
        ...
        if( m_reaction.shouldDebugBreak ) {
            ...
            // (To go back to the tests and change execution, jump over the throw, next)
...
#if !defined( CATCH_CONFIG_COLOUR_NONE ) && !defined( TKOM_CONFIG_COLOUR_WINDOWS ) && !defined( CATCH_CONFIG_COLOUR_ANSI )
#   ifdef CATCH_PLATFORM_WINDOWS
...
namespace Catch {
    ...
    void TestRegistry::registerTest( TestCase const& testCase ) {
        ...
        if( name.empty() ) {
            ReusableStringStream rss;
            rss << "Anonymous tests case " << ++m_unnamedCount;
